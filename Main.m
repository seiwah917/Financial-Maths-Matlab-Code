...
%% Simulation settings (Monte Carlo Method)
S0    = data.S0;
r     = data.r;
...
%% Simulate paths with variance reduction (antithetic variates)
% Preallocate
% Vectorization (update all M paths at once)
% Cuts reallocations and speeds up the simulation.
p      = zeros(M,1);
p_anti = zeros(M,1);
...
for t = 1:N
    % Variance update (Milstein discretisation + truncation. The max(...,0) truncates negative values from discretization error, stabilizing the Heston variance process.)
...
price  = mean(cp); % Monte Carlo estimate of the option price
